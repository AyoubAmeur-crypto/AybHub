...
const [privacyOpen, setPrivacyOpen] = useState(false);
  ...
  const [successMessage,setSuccessMessage]=useState(null)
    const [successMessageModal,setSuccessMessageModal]=useState(null)
...
try {
  ...
  if(res.data.success){
    ...
    console.log("response sent",res.data.message);
     setSuccessMessageModal(res.data.message)
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="lg:w-1/3 space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 border-t-4 border-blue-600">
                  <div className="text-center">
                    ...
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-xl font-medium flex items-center justify-center hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
                    >
                ...
                {/* Invitations Sent */}
                <div className="bg-white rounded-2xl shadow-lg p-6">
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Invitations Sent</h3>
                  <div className="space-y-3">
                    {invitations?.map((invite) => (
                      <div key={invite.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-600">{invite.post}</p>
                        </div>
...
<Lock className="w-10 h-10 mr-2 " />
    ...
    ) : null}             {(successMessageModal || alert) && (
        <div className="grid w-full max-w-xl items-start gap-4 mb-3">
          <Alert 
            variant={successMessageModal ? "default" : "destructive"}
            className={cn(
              successMessageModal ? "bg-green-600 border border-green-800/40 text-white" : "bg-red-700 border border-red-800/40 text-pink-100" )}
          >
            ...
            <AlertTitle className={cn(
              ...
              {successMessageModal ? "Success" : "Password Update Error"}
            </AlertTitle>
    ...
    teamMembers.map((member) => (
      <div key={member.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:shadow-md transition-all duration-300">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button className="text-red-600 hover:text-red-800 transition-colors" onClick={()=>{setConfiramtionModal(true) , setDeletedUser(member.member)}}>
  ...
  <div className="fixed inset-0 bg-black/30 backdrop-blur-sm animate-fade-in flex items-center justify-center p-4 z-50">
    <div className="bg-white rounded-2xl p-6 w-full max-w-md">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => {setShowInviteModal(false) , setFormData({inviteEmail:'',inviteRole:''})}}
