...
import '../../App.css'
function TaskCard({ task, setTasks,fetchTasks, columnId, onDragStart, onDragEnd, index, projectId }) {
...
const [allAvailableTeams, setAllAvailableTeams] = useState([]); // You may want to fetch this
...
setLoadingSubtask(true)
 ...
 try {
    ...
    const addSubTask = await axios.post(`http://localhost:3000/task/api/AddSubTask/${selectedTask._id}`,{item:newSubTaskName},{withCredentials:true})
    ...
    console.log("can't fetch subtask due to this",error);
...
useEffect(() => {
    ...
    const fetchMember = async () => {
      try {
        const res = await axios.post("http://localhost:3000/task/api/fetchedTaskMember", { managerId: userData.id }, { withCredentials: true })
...
// Title edit handlers
const handleTitleEdit =  () => setIsEditingTaskTitle(true);
const saveTitleEdit = async () => {
  setIsEditingTaskTitle(false);
  setEditedTaskTitle(editedTaskTitle.trim());
    setSelectedTask(prev => ({ ...prev, name: editedTaskTitle }));
      setTaskChange(true); // Mark as changed, but don't call backend!
...
const cancelTitleEdit = () => {
  setIsEditingTaskTitle(false);
  setEditedTaskTitle(selectedTask.name || '');
};

// Description edit handlers
const handleDescriptionEdit = () => setIsEditingTaskDescription(true);
const saveDescriptionEdit = async () => {
  setIsEditingTaskDescription(false);
    setSelectedTask(prev => ({ ...prev, description: editedTaskDescription }));

  setEditedTaskDescription(editedTaskDescription.trim());
    setTaskChange(true); // Mark as changed, but don't call backend!
...
// Member/Team add/remove (dummy for now)
const addMemberToTask = (user) => setTaskMembers(prev => {
...
const removeMemberFromTask = (userId) => setTaskMembers(prev => {
  
  const filtredOnes = prev.filter(p=>p._id !==userId)
  setSelectedMembers(filtredOnes)
  setTaskChange(true)
  return filtredOnes

});
const addTeamToTask = (team) => setTaskTeams(prev=>{
...
const removeTeamFromTask = (teamId) => setTaskTeams(prev=>{

  const filtredTeams = prev.filter(t => t._id !== teamId)

  setSelectedTeams(filtredTeams)
  setTaskChange(true)
  return filtredTeams
});
...
const removeSubTask = async (idx)=>{
  ...
  try {
    await axios.delete(
  `http://localhost:3000/task/api/removeSubTask/${selectedTask._id}`,
  { data: { idx: idx }, withCredentials: true }
);
    fetchSubTasksForTask()
  } catch (error) {


    console.log("can't delete task due to this",error);
...
// Upload handler for task attachments
const handleAttachmentUpdate = async (e) => {
  const files = Array.from(e.target.files);
  if (files.length === 0) return;

  if ((taskAttachments.length + files.length) > 10) {
    alert(`You can only upload up to 10 files. You currently have ${taskAttachments.length} files.`);
    return;
  }
  ...
  try {
    ...
    files.forEach(file => {
      formData.append("attachements", file); // match backend field name
    });

    const res = await axios.post("http://localhost:3000/task/api/uploadAttachments", formData, {
      headers: { "Content-Type": "multipart/form-data" },
      withCredentials: true
    });

    setTaskAttachments(prev => [...prev, ...res.data.attachments]);
    setTaskChange(true);
    e.target.value = '';
  } catch (error) {
...
// Remove handler for task attachments
const removeAttachmentUpdate = (indexToRemove) => {
  setTaskAttachments(prev => prev.filter((_, index) => index !== indexToRemove));
  setTaskChange(true);
};
// Comment handlers (dummy for now)
 ...
 const priorityBadgeMap = {
  ...
  const deleteTask = async (task) => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:3000/task/api/delete-task/${task._id}`, {
        withCredentials: true
      });
    ...
    <div className="flex flex-row items-center justify-between mb-4 mt-2">
      ...
      <div className="flex items-center gap-1 ml-2">
        ...
        <button
          ...
          className={` text-gray-400 hover:text-blue-500 transition-all duration-200 p-1 flex-shrink-0 ${isDeleting ? 'cursor-not-allowed' : ''}`}
        >
          <SquarePen size={14} />
        </button>
        <button
          ...
          disabled={isDeleting}
          className={` text-gray-400 hover:text-red-500 transition-all duration-200 p-1 flex-shrink-0 ${isDeleting ? 'cursor-not-allowed' : ''}`}
        >
          <Trash2 size={14} />
        </button>
      </div>
...
<div className="fixed inset-0 z-150 flex items-center justify-center p-4 md:p-0 bg-black/20 backdrop-blur-sm">
            ...
            <div className="px-4 py-3 border-b border-gray-200">
              ...
              <div className="flex-grow">
                  {isEditingTaskTitle ? (
                    ...
                    <h2 className="text-2xl font-semibold text-gray-900 cursor-pointer hover:text-blue-600 transition-colors"
                  ...
                  {isEditingTaskDescription ? (
                    <div className="mt-2">
                      ...
                      <div className="flex gap-2 mt-2">
                        <button onClick={saveDescriptionEdit}
                              disabled={!editedTaskDescription.trim()}

                          className="px-3 py-1 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded transition-colors">
                          change
                        </button>
                        <button onClick={cancelDescriptionEdit}
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-600 cursor-pointer hover:text-blue-600 transition-colors min-h-[20px]"
                        onClick={handleDescriptionEdit}
                        title="Click to edit">
                        {editedTaskDescription || "Click to add description..."}
                      </p>
                    </div>
                  )}
                  <div className="mt-5">
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar">
              <div className="p-6 flex gap-6">
                ...
                <div className="w-2/3 space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex space-x-2 pt-2">
                      <div className="relative">
                        <button
                          onClick={() => setShowManageTaskMembersModal(true)}
                        ...
                        {showManageTaskMembersModal && (
                          <>
                            <div 
                              className="fixed inset-0 z-[9998] animate-fade-in" 
                              onClick={() => setShowManageTaskMembersModal(false)}
                            ></div>
                                  ...
                                  <div className="relative">
                                    <div className="border border-gray-300 rounded-lg shadow-sm focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-blue-600 transition-all">
                                          ...
                                          <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto modal-scrollbar">
                                            {taskMembers.map((member) => (
                                              <div key={member._id} className="flex items-center gap-1 bg-blue-100 text-blue-800 px-0.5 py-0.5 rounded-full text-xs flex-shrink-0">
                                                ...
                                                <button 
                                                  onClick={() => removeMemberFromTask(member._id)}
                                      ...
                                      <div className="p-2">
                                        <input 
                                          type="text"
                                          placeholder={taskMembers.length > 0 ? "Add more members..." : "Search and add members..."}
                      ...
                      <div className="relative">
                        <button
                          onClick={() => setShowManageTaskTeamsModal(true)}
                          className="flex items-center space-x-1 px-3 py-1.5 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md transition-colors"
                        >
                          <Settings className="w-4 h-4" />
                          <span>Manage Teams</span>
                        </button>
                        {showManageTaskTeamsModal && (
                          <>
                            <div 
                              className="fixed inset-0 z-[9998] animate-fade-in" 
                              onClick={() => setShowManageTaskTeamsModal(false)}
                            ></div>
                            <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-200 z-[9999] max-h-72 overflow-hidden animate-fade-in">
                              <div className="p-3">
                                <div className="flex items-center justify-between mb-3">
                                  ...
                                  <div className="relative">
                                    <div className="border border-gray-300 rounded-lg shadow-sm focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-blue-600 transition-all">
                                          ...
                                          <div className="flex flex-wrap gap-1 h-8 overflow-y-auto modal-scrollbar">
                                            {taskTeams.map((team) => (
                                              <div key={team._id} className="flex items-center gap-1 bg-blue-100 text-blue-800 px-0.5 py-0.5 rounded-full text-xs flex-shrink-0">
                                                <div className={`w-7 h-7  rounded-full flex flex-col items-center justify-center bg-blue-600 text-white text-[11px] font-semibold`}>
                                                  {getTeamInitials(team.name)}
                                                </div>
                                                <span className="text-xs truncate max-w-16">{team.name.split(' ')[0]}</span>
                                                <button 
                                                  onClick={() => removeTeamFromTask(team._id)}
                                                  className="hover:text-blue-900"
                                                >
                                                  <CircleX className="w-2.5 h-2.5" />
                                                </button>
                                      ...
                                      <div className="p-2">
                                        <input 
                                          type="text"
                                          placeholder={taskTeams.length > 0 ? "Add more teams..." : "Search and add teams..."}
                                          className="w-full outline-none text-xs placeholder-gray-500"
                                        />
                                      </div>
                                    </div>
                                ...
                                <div className="mb-2">
                                  ...
                                  <div className="space-y-1 max-h-32 overflow-y-auto modal-scrollbar pb-4">
                                    {allAvailableTeams.length === 0 ? (
                                      ...
                                      allAvailableTeams
                                        .filter(team => !taskTeams.find(t => t._id === team._id))
                                        .map((team) => (
                                        <div 
                                          key={team._id} 
                                          className="flex items-center justify-between p-1.5 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer group"
                                          onClick={() => addTeamToTask(team)}
                                        >
                                          <div className="flex items-center space-x-2">
                                            <div className={`w-8 h-8  rounded-full flex items-center justify-center text-white font-bold text-xs bg-gradient-to-br from-blue-600 to-blue-700`}>
                                              {getTeamInitials(team.name)}
                                            </div>
                                            <div>
                                              <p className="text-xs font-medium text-gray-900 truncate max-w-24">{team.name}</p>
                                            </div>
                                          </div>
                                          <Plus className="w-3 h-3 text-blue-600 opacity-0 group-hover:opacity-100 transition-all" />
                                        </div>
                  ...
                  <div className="min-h-fit overflow-y-auto custom-scrollbar pr-4 w-full animate-fade-in" key={activeTab}>
  ...
  <div className="w-full space-y-4">
    ...
    {/* Styled Subtask Modal */}
    {showSubTaskModal && (
      <div className="mt-2 w-full animate-fade-in">
        <div className="bg-white border-2 border-gray-200 border-dashed rounded-xl p-6   transition-all duration-200">
          {/* Header */}
          <div className="flex items-center justify-between mb-7">
            <h3 className="text-md font-normal text-gray-900">Add Subtask</h3>
            <button
              onClick={() => {setShowSubTaskModal(false) , setNewSubTaskName('')}}
          ...
          <div className="">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base hover:border-blue-600 transition-colors"
                placeholder="Enter subtask name..."
              />
              <button
                className="px-3 py-2 text-md font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors flex items-center"
    ...
    <p className="text-sm text-gray-400 text-center">
      Start by adding your first subtask!
    </p>
  </div>
            ) : (
        ...
        className="absolute right-2 top-1/2 mt-2.5 -translate-y-1/2 p-1 rounded-full bg-white text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors shadow-sm z-10 flex items-center justify-center"
                      ...
                      <div className="space-y-3 mr-2">
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-blue-400 transition-colors">
                          <input
                            ...
                            disabled={isUploading || (taskAttachments?.length >= 5)}
                          />
                            ...
                            {isUploading ? (
                              ...
                              <>
                                ...
                                <p className="text-sm text-gray-600">
                                  {(taskAttachments?.length >= 5) ? 'Maximum 5 files reached' : 'Upload new attachment'}
                                </p>
                                <p className="text-xs text-gray-400">
                                  Max 5 files, up to 5MB each ({5 - (taskAttachments?.length || 0)} remaining)
                                </p>
                              </>
                            )}
                          </label>
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              {taskAttachments.length > 0 && (
                                <button
                                  onClick={async () =>{ setTaskAttachments([])
                                    setTaskChange(true)
                                  }}
...
>
    {loading ? (
      ...
      {sure && taskDeleted && (
        <div key={sure} className="fixed inset-0 z-[10003] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm animate-fade-in">
          <div 
          ...
          ></div>
          <div key={sure} className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full relative z-10 animate-fade-in">
      ...
      {warningModal && (
        <div className="fixed inset-0 z-[10004] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-fade-in">
          <div 
            className="fixed inset-0" 
            onClick={() => setWarningModal(false)}
          ></div>
          <div key={warningModal} className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full relative z-10 border border-gray-200 animate-fade-in">
            <div className="flex items-center mb-4">
