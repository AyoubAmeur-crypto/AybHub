...
import TaskWorkerBoard from './TaskWorkerBoard'
import TaskAdminBoard from '../admin/TaskAdminBoard'
import Loading from '../Loading'


function TaskWorkerDashboard() {
      const [allProject,setAllProject]=useState([])
      const [slectedProjectId,setSelectedProjectId]=useState(null)
      const [switchProject,setSwitchProject]=useState(false)
      const [isProjectManager,setIsProjectManager]=useState(null)
      ...
      const fetchAllProject = async ()=>{
        ...
        console.log(availableProject.data);
        

        if(availableProject.data.length > 0 && !slectedProjectId){

            setSelectedProjectId(availableProject.data[0]._id)
            setIsProjectManager(availableProject.data[0].manager._id); // <-- Add this line
      ...
      const handlePoject = (project)=>{

        setSelectedProjectId(project._id)
        setIsProjectManager(project.manager._id)
  ...
  <div className="relative min-h-screen pb-20 bg-gray-50">
    ...
    {allProject.length === 0 ? (
      <div className="flex flex-col items-center justify-center min-h-[550px]">
        ...
        <p className="text-gray-400 text-base">You have no assigned projects.</p>
      </div>
    ) : (
      <>

      {slectedProjectId && isProjectManager && userData.id &&
  (isProjectManager.toString() === userData.id.toString()
    ? <TaskAdminBoard projectId={slectedProjectId} />
    : <TaskWorkerBoard projectId={slectedProjectId} />
  )
}      
...
export default TaskWorkerDashboard
