...
const TaskCompletedChartCard = ({ className , dueTodayTasks ,TasksInfos}) => {

  
  const doneTask = TasksInfos.filter(t => t.status==='done')
  const percent = TasksInfos.length === 0 ? 0 : Math.round((doneTask.length / TasksInfos.length) * 100);
        ...
        <div className="w-full flex items-center justify-between px-8 pt-4 pb-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="px-2.5 py-1 rounded-xl bg-gradient-to-r from-green-100 to-green-200 text-green-800 text-sm font-bold shadow-sm">
              {doneTask.length}/{TasksInfos.length}
            </span>
          </div>
        </div>
    ...
    <div className="relative z-10">
      ...
      <div className="flex-1 space-y-1.5">
        {recentNotifications.map(activity => (
          <div key={activity.id} className="group/activity flex items-start h-[85px] bg-white/80 backdrop-blur-sm rounded-2xl shadow-md border border-gray-100/80 p-3 hover:shadow-lg hover:bg-white transition-all duration-300 animate-fadein-pop">
...
const fetchGeneralCalrndarInfos = async ()=>{
    ...
    const fetchTaskInfos = async ()=> {
      ...
      try {
        ...
        const res = await axios.get("http://localhost:3000/dashboard/getTaskInfo",{withCredentials:true})
        console.log("tasks vailable for this user ",res.data.tasks);
        
        setTasksInfos(res.data.tasks)
        
      } catch (error) {

        console.log("can't ftch task due to this",error);
  ...
  const fetchGeneralInfos = async ()=>{
    ...
    const fetchTeamInfos = async ()=>{
      ...
      try {
        ...
        console.log("can't fetch teams due tot this",error);
...
const dueTodayTasks = TasksInfos.filter(t => {
  ...
  const statCardsData = [
    ...
    { icon: <Users2 className="w-6 h-6 text-white" />, title: "JOINED TEAMS", value: Array.isArray(teams) ? teams.length : 0, trend: "up", trendValue: "+1" },
  ];
    ...
    <div className="h-[580px] w-full flex flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3 overflow-hidden">
        ...
        <div className="flex flex-col gap-2 h-[430px]">
          <TaskCompletedChartCard dueTodayTasks={dueTodayTasks} TasksInfos={TasksInfos} className="h-[200px]" />
